...
print("divides into different folders with labels")
# Paths for input and output directories
input_folder = './data/processed_img'  # Original dataset with all classes
output_folder = './data/binaryLabels_img'   # Folder for binary classification (food vs. other)
...
# Step 2: Create "food" and "other" directories with their class subfolders in train, val, and test
def organize_binary_folders(base_dir):
    for split in ['train', 'val', 'test']:
        ...
        for class_folder in os.listdir(split_dir):
            ...
            # Create a class subfolder in the target directory ('food' or 'other')
            class_target_dir = os.path.join(target_dir, class_folder)
            os.makedirs(class_target_dir, exist_ok=True)
            
            # Move all files from class folder to the respective class subfolder in 'food' or 'other'
            if os.path.isdir(class_path):
                for file_name in os.listdir(class_path):
                    src_file = os.path.join(class_path, file_name)
                    dst_file = os.path.join(class_target_dir, file_name)
                    shutil.move(src_file, dst_file)
                
                # Remove the now-empty class folder
                os.rmdir(class_path)

# Step 3: Organize the data into 'food' and 'other' folders with subfolders for each category
organize_binary_folders(output_folder)

print("Dataset successfully organized for binary classification (food vs. other) with class subfolders.SplitData2.py")
